...
def main() -> int:
    ...
    result, command_properties = worker_manager.WorkerProperties.create(
        ...
        work_arguments=(connection, DRONE_TARGET),
    ...
    while time.time() - start_time < 100 and drone_connected:
        ...
        try:
            heartbeat_status: str = heartbeat_receiver_to_main_queue.queue.get_nowait()
            main_logger.info(f"Heartbeat status: {heartbeat_status}")
            if heartbeat_status == "Disconnected":
                drone_connected = False
                main_logger.info("Drone disconnected, shutting down")
                break
        except queue.Empty:
            pass

        # Log any commands issued
        try:
            command_str: str = command_to_main_queue.queue.get_nowait()
